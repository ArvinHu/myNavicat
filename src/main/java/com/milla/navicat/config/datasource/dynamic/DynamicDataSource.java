    ...
    public boolean createDataSource(String key, String driveClass, String url, String username, String password, String databaseType) {
        try {
            if (!testDatasource(driveClass, url, username, password)) {
                throw new DataSourceException("数据源配置有错误", 500);
            }
...
//                driveClass = "com.mysql.cj.jdbc.Driver";//DBUtil.mysqldriver;
...
//        }
        if (testDatasource(driveClass, url, username, password)) {
            boolean result = this.createDataSource(datasourceId, driveClass, url, username, password, databaseType.getCategory());

            if (!result) {
                throw new DataSourceException("数据源" + datasourceId + "配置正确，但是创建失败", 500);
            }
        } else {
            throw new DataSourceException("数据源配置有错误", 500);
        }
